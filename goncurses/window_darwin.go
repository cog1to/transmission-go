...
func (w *Window) GetWChar() Key {
  var ch int
	status := C.wget_wch(w.win, (*C.wint_t)(unsafe.Pointer(&ch)))
	if status == C.ERR {
		ch = 0
	}
	return Key(ch)
...
// supplied message. See Print for more details.The first two arguments are the
// coordinates to print to.
func (w *Window) MovePrintW(y, x int, str wchar.WcharString) {
	C.mvwaddwstr(w.win, C.int(y), C.int(x), (*C.wchar_t)(unsafe.Pointer(str.Pointer())))
}
